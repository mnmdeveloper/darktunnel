...
def user_menu(active: bool) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[[button("🔑 Запросить доступ", "user:request_access")]])
...
async def user_request_access(c: CallbackQuery) -> None:
    ...
    for admin_id in admins:
        if not admin_id:
            continue
        try:
            await c.bot.send_message(admin_id, text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[button("👤 Открыть пользователя", f"mg:user:view:{user.id}")], [button("💳 Подписки", "subscription:admin")]]))
            sent += 1
        except Exception:
...
async def user_subscription_link(c: CallbackQuery) -> None:
    ...
    if c.message:
        await c.message.answer(f"<b>🔗 Ваша ссылка управления DarkTunnel</b>\n\n<code>{make_access_link(token)}</code>", parse_mode="HTML")
...
async def admin_subscription_link_send(c: CallbackQuery, bot: Bot) -> None:
    ...
    try:
        await bot.send_message(telegram_id, "🔗 <b>Ваша ссылка управления DarkTunnel</b>\n\nОткройте её в приложении:\n\n" + f"<code>{escape(link)}</code>", parse_mode="HTML")
