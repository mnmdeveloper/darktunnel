...
async def show_user(c: CallbackQuery, user_id: str) -> None:
    # Answer immediately: Telegram's callback spinner must never depend on DB latency.
    try:
        await c.answer()
    except Exception:
        pass
    try:
        ...
        if c.message:
            await c.message.edit_text(
                f"<b>⚠️ Не удалось открыть пользователя</b>\n\n<code>{escape(str(exc)[:500])}</code>",
                reply_markup=kb([[b("⬅️ Пользователи", "users:0")]]),
                parse_mode="HTML",
            )
        return
    if not user:
        if c.message:
            await c.message.edit_text("<b>Пользователь не найден</b>", reply_markup=kb([[b("⬅️ Пользователи", "users:0")]]), parse_mode="HTML")
        return
