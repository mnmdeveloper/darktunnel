...
async def activation_server_profile_for_node(
    ...
    """Return one published server's private client profile to an activated device.

    The public /v1/servers directory intentionally never contains the AWG client
    private key. This authenticated endpoint is the per-server equivalent of the
    legacy primary-server profile endpoint.
    """
    try:
        payload = decode_activation_token(token)
        activation_id = str(payload["activation_id"])
