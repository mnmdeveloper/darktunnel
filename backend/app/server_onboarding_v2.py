...
async def connect(credentials: SSHCredentials) -> asyncssh.SSHClientConnection:
    ...
    if credentials.private_key:
        try:
            keys = [asyncssh.import_private_key(credentials.private_key)]
        except (asyncssh.KeyImportError, ValueError) as exc:
            raise ValueError("Не удалось прочитать приватный SSH-ключ") from exc
...
async def _run(
    ...
    if username == "root":
        ...
        result = await asyncio.wait_for(connection.run(wrapped, check=False), timeout=timeout)
    elif password:
        wrapped = f"sudo -S -p '' bash -lc {shlex.quote(command)}"
        result = await asyncio.wait_for(
            connection.run(wrapped, input=password + "\n", check=False), timeout=timeout
        )
    else:
...
async def install_and_discover(
    ...
    branch: str = "server-onboarding-v2",
) -> dict[str, Any]:
    async with await connect(credentials) as connection:
        actual = _fingerprint(connection.get_server_host_key())
        ...
        await _run(
            connection,
            f"curl -fsSL {shlex.quote(url)} | {env} bash -s -- install",
            credentials.password,
            timeout=900,
        )
        token = await _run(
            connection,
            "python3 -c 'import json; print(json.load(open(\"/etc/darktunnel-node/node.json\"))[\"management_token\"])'",
            credentials.password,
        )
        status = await _run(
            connection,
            f"curl -fsS -H {shlex.quote('Authorization: Bearer ' + token)} http://127.0.0.1:8787/v1/status",
            credentials.password,
        )
...
async def register_discovery(
    ...
    node = await session.scalar(
        select(ServerNode).where(ServerNode.host == host, ServerNode.archived_at.is_(None))
    )
    safe_node_config = {
        "node_id": discovery.get("node_id", ""),
        "management_token": discovery.get("_management_token", ""),
    ...
    if node is None:
        node = ServerNode(
            name=draft.name,
            country_name=draft.country,
            city=draft.city,
            host=host,
            port=0,
            protocol_mode="multi",
            encrypted_config=encrypt_server_config(safe_node_config),
            published=False,
            auto_select=True,
            maintenance=False,
        )
    ...
    for transport_type in (TransportType.amneziawg2, TransportType.wdtt):
        ...
        row = await session.scalar(
            select(ServerTransport).where(
                ...
                ServerTransport.transport_type == transport_type,
            )
        )
        if row is None:
        ...
        row.online = detected
        row.published = detected
        row.status_detail = json.dumps(info, ensure_ascii=False)[:8000]
        row.last_checked_at = datetime.now(UTC)
        row.port = 56000 if transport_type == TransportType.wdtt else 0
        any_online = any_online or detected
    ...
    job.progress = 100
    job.detail = "Server discovered and registered"
    session.add(
        AuditLog(
            admin_id=draft.admin_id,
            action="server.onboard",
            entity_type="server",
