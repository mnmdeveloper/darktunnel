...
struct HomeView: View {
        ...
        VStack(spacing: 10) {
            HStack(spacing: 11) {
                Image(systemName: "shield.lefthalf.filled")
                    ...
                    .frame(width: 40, height: 40).background(accent.opacity(0.18), in: Circle())
                ...
                Button { showingSettings = true } label: {
                    Image(systemName: "gearshape.fill").foregroundStyle(.white)
                        .frame(width: 38, height: 38).background(panel.opacity(0.92), in: Circle())
                }
            }
            ...
            HStack(spacing: 8) {
                ...
                .opacity(viewModel.state == .connected ? 1 : 0.45)
            }
        }
        .padding(14)
    ...
    private var subscriptionStatus: String { ActivationStore.shared.isActivated ? "Подписка активна" : "Требуется активация" }
