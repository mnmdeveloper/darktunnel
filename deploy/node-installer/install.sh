...
if path.exists():
    try:
        print(json.loads(path.read_text(encoding="utf-8")).get(sys.argv[2], ""))
    except Exception:
        pass
PY
}
...
[Unit]
Description=DarkTunnel read-only VPN node discovery agent
...
UNIT
  ...
  for _ in $(seq 1 30); do
    if systemctl is-active --quiet darktunnel-node.service && curl -fsS http://127.0.0.1:8787/health >/dev/null 2>&1; then return; fi
    sleep 1
  done
