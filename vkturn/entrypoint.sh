...
ip link set wg0 up
# net.ipv4.ip_forward is configured by Docker Compose for this namespace.
# Writing /proc/sys here fails on hosts where the container sysctl mount is read-only.
...
exec /usr/local/bin/vk-turn-proxy -listen 0.0.0.0:56100 -connect 127.0.0.1:51820 -srtp
